...
const utils = {
  ...
  async sendMainMenu(ctx, userId) {
    ...
    const keyboard = Markup.inlineKeyboard([
      ...
      CONFIG.ADMIN_IDS.includes(userId) ? [Markup.button.callback('⚙️ Админ-панель', 'admin')] : []
    ]);
    return utils.sendOrEditMessage(ctx, messageText, keyboard, userId);
  },
...
bot.use(async (ctx, next) => {
  ctx.session = ctx.session || { previousScreen: null };
...
bot.on('callback_query', async (ctx) => {
  ...
  if (!user && action !== 'check_sub') {
    await ctx.answerCbQuery(`❌ Пользователь не найден! Напиши /start в ${CONFIG.BOT_NAME}.`, { show_alert: true });
    return;
  }
  ...
  try {
    if (action === 'check_sub') {
      if (!(await utils.isUserSubscribed(ctx))) {
        await ctx.answerCbQuery(`❌ Подпишись на ${CONFIG.REQUIRED_CHANNEL} для доступа к ${CONFIG.BOT_NAME}!`, { show_alert: true });
        return;
      }
      const existing = db.get('SELECT * FROM users WHERE id = ?', [id]);
      if (!existing) {
    ...
    if (action === 'farm') {
      ...
      const updatedStars = db.get('SELECT stars FROM users WHERE id = ?', [id]).stars;
      await ctx.reply(
        `⭐ +1 звезда! Твой баланс: ${updatedStars} звёзд.`,
        { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🏠 В главное меню', 'back')]]) }
      );
      ctx.session.previousScreen = null;
      await utils.sendMainMenu(ctx, id);
    ...
    if (action === 'bonus') {
      ...
      const updatedStars = db.get('SELECT stars FROM users WHERE id = ?', [id]).stars;
      await ctx.reply(
        `🎉 +5 звёзд! Твой баланс: ${updatedStars} звёзд.`,
        { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🏠 В главное меню', 'back')]]) }
      );
      ctx.session.previousScreen = null;
      await utils.sendMainMenu(ctx, id);
    ...
    if (action.startsWith('check_task_')) {
      ...
      if (userTask.completed) {
        await ctx.answerCbQuery(`✅ Задание уже выполнено! Перейди к следующему в ${CONFIG.BOT_NAME}! 🌟`, { show_alert: true });
        return;
      }
      ...
      await ctx.reply(
        ...
        { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🚫 Отменить', 'cancel_input')]]) }
      );
      await ctx.answerCbQuery();
      return;
    }
    ...
    if (action === 'titles') {
      ...
      ctx.session.previousScreen = 'faq';
      await utils.sendOrEditMessage(
        ctx,
        ...
        Markup.inlineKeyboard([
          [Markup.button.callback('🔙 Назад', 'faq'), Markup.button.callback('🏠 В главное меню', 'back')]
        ]),
        id
      );
      await ctx.answerCbQuery();
    ...
    if (action === 'admin') {
      if (!CONFIG.ADMIN_IDS.includes(id)) {
        await ctx.answerCbQuery(`⛔ Доступ только для админов ${CONFIG.BOT_NAME}!`, { show_alert: true });
        return;
      }
      ctx.session.previousScreen = 'main';
      await utils.sendOrEditMessage(
    ...
    if (action.startsWith('approve_task_')) {
      ...
      await utils.sendOrEditMessage(
        ...
        Markup.inlineKeyboard([[Markup.button.callback('🔙 Назад', 'admin_tickets'), Markup.button.callback('🏠 В главное меню', 'back')]]),
        id
      );
    ...
    if (action.startsWith('set_ticket_status_')) {
      ...
      if (!['in_progress', 'closed'].includes(status)) {
        await ctx.answerCbQuery('❌ Неверный статус!', { show_alert: true });
        return;
      }
    ...
    if (action === 'back') {
      ...
      if (previousScreen === 'main') {
        ...
        await ctx.telegram.sendCallbackQuery({ id: ctx.callbackQuery.id, data: previousScreen });
      }
      await ctx.answerCbQuery();
      return;
    }
    ...
    if (action === 'admin_tickets') {
      ...
      buttons.push([Markup.button.callback('🔙 Назад', 'admin'), Markup.button.callback('🏠 В главное меню', 'back')]);
      ctx.session.previousScreen = 'admin';
      await utils.sendOrEditMessage(
        ctx,
        `📞 <b>Тикеты и заявки</b>\n\nВыбери тикет или заявку для обработки: 🔍`,
...
bot.on('message', async (ctx) => {
  ...
  for (const user of users) {
    try {
      await ctx.telegram.sendMessage(
        user.id,
        `📢 <b>Новость от ${CONFIG.BOT_NAME}</b>\n\n${messageText}`,
        { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🏠 В главное меню', 'back')]]) }
      );
      successCount++;
    } catch (err) {
...
if (ctx.session.waitingFor?.type === 'add_promo' && CONFIG.ADMIN_IDS.includes(id)) {
  const [code, reward, activations] = ctx.message.text.trim().split(/\s+/);
  ...
  db.run('INSERT OR IGNORE INTO promo_codes (code, reward, activations_left) VALUES (?, ?, ?)', [code, parseInt(reward), parseInt(activations)]);
  ctx.session.waitingFor = null;
  ctx.session.previousScreen = 'admin';
  await ctx.reply(
...
if (ctx.session.waitingFor?.type === 'add_task' && CONFIG.ADMIN_IDS.includes(id)) {
  const [type, ...rest] = ctx.message.text.trim().split(/\s+/);
  const description = rest.slice(0, -2).join(' ');
  const [goal, reward] = rest.slice(-2);
  ...
  db.run('INSERT OR IGNORE INTO tasks (type, description, goal, reward) VALUES (?, ?, ?, ?)', [type, description, parseInt(goal), parseInt(reward)]);
  ctx.session.waitingFor = null;
  ctx.session.previousScreen = 'admin';
  await ctx.reply(
...
if (ctx.session.waitingFor?.type === 'manage_stars' && CONFIG.ADMIN_IDS.includes(id)) {
  const [userId, stars] = ctx.message.text.trim().split(/\s+/).map(Number);
  ...
  if (!targetUser) {
    ...
    ctx.session.previousScreen = 'admin';
    await ctx.reply(
      `❌ <b>Пользователь ${userId} не найден!</b>`,
      {
        parse_mode: 'HTML',
        ...Markup.inlineKeyboard([
          [Markup.button.callback('🔙 Назад', 'admin'), Markup.button.callback('🏠 В главное меню', 'back')]
  ...
  db.run('UPDATE users SET stars = stars + ? WHERE id = ?', [stars, userId]);
  utils.updateUserTitle(ctx, userId);
  const updatedStars = db.get('SELECT stars FROM users WHERE id = ?', [userId]).stars;
  await ctx.telegram.sendMessage(
    userId,
    `⭐ <b>Обновление баланса!</b>\n\n` +
    `Твой баланс изменился на ${stars > 0 ? '+' : ''}${stars} звёзд.\n` +
    `Новый баланс: ${updatedStars} ⭐`,
    { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🏠 В главное меню', 'back')]]) }
...
if (ctx.session.waitingFor?.type === 'manage_titles' && CONFIG.ADMIN_IDS.includes(id)) {
  const [userId, titleId] = ctx.message.text.trim().split(/\s+/).map(Number);
  ...
  if (titleId === 0) {
    ...
    await ctx.telegram.sendMessage(
      userId,
      `🏅 <b>Титул снят!</b>\n\nТеперь у тебя нет титула. Продолжай играть в ${CONFIG.BOT_NAME}! 🌟`,
      { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🏠 В главное меню', 'back')]]) }
    ).catch(err => console.error(`Ошибка уведомления пользователя ${userId}:`, err));
    ctx.session.waitingFor = null;
    ctx.session.previousScreen = 'admin';
  ...
  if (!title) {
    ...
    ctx.session.previousScreen = 'admin';
    await ctx.reply(
      `❌ <b>Титул с ID ${titleId} не найден!</b>`,
      {
        parse_mode: 'HTML',
        ...Markup.inlineKeyboard([
          [Markup.button.callback('🔙 Назад', 'admin'), Markup.button.callback('🏠 В главное меню', 'back')]
  ...
  await ctx.telegram.sendMessage(
    userId,
    `🏅 <b>Новый титул!</b>\n\n` +
    `Ты получил: <b>${title.name}</b> (${title.description}) 🌟`,
    { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🏠 В главное меню', 'back')]]) }
  ).catch(err => console.error(`Ошибка уведомления пользователя ${userId}:`, err));
  ctx.session.waitingFor = null;
  ctx.session.previousScreen = 'admin';
